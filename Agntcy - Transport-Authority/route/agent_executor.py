...
from route.agent import RouteAgent

logger = logging.getLogger("mbta.route_agent.a2a_executor")

class RouteAgentExecutor(AgentExecutor):
    """
    This class extends the base `AgentExecutor` and executes requests on behalf of the Route 
    Agent in an Agent-to-Agent (A2A) architecture.

    This executor handles user prompts related to MBTA routes. It validates incoming requests, interacts with the RouteAgent for 
    planning routes, and publishes appropriate events (e.g., messages or tasks) to the event queue.
    ...
    async def execute(
        ...
        """
        Processes a user prompt to generate an MBTA route via the RouteAgent.

        This method extracts the user prompt from the request context, invokes the RouteAgent
        asynchronously to get an MBTA route, and publishes the result as an event.
        ...
        Raises:
            ServerError: If an unexpected error occurs during an MBTA route.
        """
        ...
        try:
            ...
            if output.get("error_message") is not None and output.get("error_message") != "":
                ...
                message = new_agent_text_message(
                    output.get("error_message", "Failed to generate MBTA route"),
                )
                await event_queue.enqueue_event(message)
                return

            route = output.get("route_notes", "No routes returned")
            logger.info("Route profile generated: %s", route)
            await event_queue.enqueue_event(new_agent_text_message(route))
        except Exception as e:
            logger.error(f'An error occurred while streaming the route profile response: {e}')
