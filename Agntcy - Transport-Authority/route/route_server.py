...
from route.agent_executor import RouteAgentExecutor
from route.card import AGENT_CARD

load_dotenv()

# Initialize a multi-protocol, multi-transport gateway factory.
factory = AgntcyFactory("mbta.route_agent", enable_tracing=True)

async def main():
    """
    Starts the route agent server using the specified transport mechanism.

    This function initializes a RouteAgentExecutor wrapped with a DefaultRequestHandler,
    ...
    - ROUTE_AGENT_HOST / ROUTE_AGENT_PORT: Host and port for local HTTP server (if "A2A" is selected)
    """
    ...
    if DEFAULT_MESSAGE_TRANSPORT == "A2A":
        config = Config(app=server.build(), host=ROUTE_AGENT_HOST, port=ROUTE_AGENT_PORT, loop="asyncio")
        ...
        # Create an application session for the route agent
        app_session = factory.create_app_session()
        
        # Add the route agent container to the session with the specified transport
        app_session.add_app_container("mbta-route", AppContainer(
            server,
            transport=transport,
            topic=A2AProtocol.create_agent_topic(AGENT_CARD),
        ))

        await app_session.start_session("mbta-route", keep_alive=True)
